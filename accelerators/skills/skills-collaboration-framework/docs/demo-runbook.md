...
- Workspaces (a dev→staging→prod SDLC):
  - **DEV** (Tier 1 self-upload): `https://greenwood-dev.cloud.databricks.example` (`greenwood-dev`)
  - **STAGING** (Tier 2 target): `https://greenwood-staging.cloud.databricks.example` (`greenwood-staging`)
  - **PROD** (part of the Tier 3 fan-out): `https://greenwood-prod.cloud.databricks.example` (`greenwood-prod`)
...
- Git remote: `azure` → ADO (`https://dev.azure.com/greenwood-demo/skills/_git/skills-collaboration-framework`)
...
| **Multiple workspaces per domain** (common enterprise topology) | `[ws_a, ws_b, …]` | `[every workspace]` | A T2 skill fans out to all of the domain's workspaces; T3 fans out to all. `sync-skills.sh`/`prune-deprecated.sh` iterate the list. |
...
> [!NOTE]
> **Example domains:** the four Tier-2 domains (`platform`, `analytics`, `finance`, `governance`)
> are neutral cross-industry examples. Each ships an illustrative Tier-2 skill; `catalog-navigator`
> is the one enterprise Tier-3 skill that applies across all domains. Every example skill is scoped
> to aggregate / operational / metadata data (`pii: false`, read-only) — the framework's §2.2 tier
> rule forces any PII/regulated skill to Tier 3, so a T2 skill carrying such data would fail the gate.
...
2. Verify `tier1/dana.lee/hello-skills/SKILL.md` exists
...
5. Merge the PR
6. Verify: the **Sync** stage does **not** publish `hello-skills` (T1 path not in the sync trigger paths)
7. Verify: `/.assistant/skills/` (STAGING workspace) has no `hello-skills` folder
...
> user until it's promoted through the process you're about to see. That's the safety guarantee:
...
2. Show `tier2/platform/pipeline-cost-analyzer/SKILL.md` — point out name + description only
...
6. **Show reviewer auto-request:** the branch policy's "Automatically included reviewers" for `tier2/platform/` requests the domain steward
...
9. Watch the merge build's **Sync** stage → `sync-skills.sh 2 platform` runs (publishing to the STAGING workspace), then the **Report eval_score** step attaches an "AI Skill Review" report to the build summary and tags the build `eval-<skill>-<score>` (the score lives in the build, never written back to the protected `main` branch)
10. Show workspace: `databricks workspace list /.assistant/skills` (STAGING) → `pipeline-cost-analyzer` appears at the **top level** (flat — the `tier2/platform/` structure lives only in Git, never in the workspace; `TESTS.yaml` is stripped by the sync)
...
1. Open the **STAGING** workspace: `https://greenwood-staging.cloud.databricks.example` (Tier 2 target)
...
2. `git mv tier2/platform/pipeline-cost-analyzer tier3/pipeline-cost-analyzer`
3. Update `registry.yaml`:
   - `tier: 2` → `tier: 3`
   - `approvals.council:` and `approvals.security:` — set each to an **authorized approver**
     from [`config/governance.yaml`](../config/governance.yaml) (e.g. council `priya.nair@greenwood.example`,
     security `marcus.chen@greenwood.example`). A name not on the allowlist fails the build —
...
6. Open a new Genie Code thread **in PROD** (`https://greenwood-prod.cloud.databricks.example`) — the
...
> "Imagine a consumer-role app — a finance analyst with **no workspace, no table access** describes a
> skill in plain language ('flag cost centers trending over budget this quarter'). An agent
