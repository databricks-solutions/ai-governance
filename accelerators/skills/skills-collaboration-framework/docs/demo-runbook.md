...
- Workspaces (a dev→staging→prod SDLC):
  - **DEV** (Tier 1 self-upload): `https://dbc-fb813e75-e5db.cloud.databricks.com` (`greenwood-demo-dev`)
  - **STAGING** (Tier 2 target): `https://dbc-e1d9ab39-ffcf.cloud.databricks.com` (`greenwood-demo-staging`)
  - **PROD** (part of the Tier 3 fan-out): `https://dbc-fbcbb704-ba3c.cloud.databricks.com` (`greenwood-demo-prod`)
...
- Git remote: `azure` → ADO (`https://dev.azure.com/cedricboisvert/cicd-demo/_git/skills-collaboration-framework`)
...
| **Multiple workspaces per domain** (a real VCH case) | `[ws_a, ws_b, …]` | `[every workspace]` | A T2 skill fans out to all of the domain's workspaces; T3 fans out to all. `sync-skills.sh`/`prune-deprecated.sh` iterate the list. |
...
> [!NOTE]
> **VCH-aligned domains:** the eleven Tier-2 domains (`acute`, `community`, `dgp`, `faa`, `aa`,
> `ed`, `lmps`, `maa`, `quality-analytics`, `infrastructure`, `eabi`) mirror VCH's team structure
...
> bureaucracy.
>
> All seven read `greenwood_dbw_catalog.vch_cerner` (a real Cerner-shaped catalog modelling VCH's
...
2. Verify `tier1/cedric.boisvert/hello-databricks/SKILL.md` exists
...
1. Open the **STAGING** workspace: `https://dbc-e1d9ab39-ffcf.cloud.databricks.com` (Tier 2 target)
...
3. Update `registry.yaml`:
   ...
   - `approvals.council:` and `approvals.security:` — set each to an **authorized approver**
     from [`config/governance.yaml`](../config/governance.yaml) (e.g. council `cedric.boisvert@databricks.com`,
     security `siddesh.ujjni@databricks.com`). A name not on the allowlist fails the build —
...
6. Open a new Genie Code thread **in PROD** (`https://dbc-fbcbb704-ba3c.cloud.databricks.com`) — the
