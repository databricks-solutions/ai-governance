...
2. **Query `main.analytics.model_metrics`** for quality metrics by model, version, and date.
3. **Join `main.analytics.inference_logs`** for request volumes when throughput is asked.
...
FROM main.analytics.model_metrics
...
FROM main.analytics.inference_logs
...
- `main.analytics.model_metrics` — per-model, per-version quality metrics by date (aggregate)
- `main.analytics.inference_logs` — serving request counts by model and date (aggregate)
