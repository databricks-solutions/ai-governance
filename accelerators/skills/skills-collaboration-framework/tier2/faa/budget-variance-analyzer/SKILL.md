...
2. **Query `main.finance.actuals`** for actual spend by cost center, account, and period.
3. **Join `main.finance.budgets`** on the same keys to compute variance (actual − budget).
...
FROM main.finance.actuals a
JOIN main.finance.budgets b
...
- `main.finance.actuals` — actual spend rolled up by cost center, account, and fiscal period
- `main.finance.budgets` — budget amounts on the same keys
