...
description: Profile the 31 AHRQ Elixhauser comorbidity flags and van Walraven weighted index
  for any Greenwood EHR encounter cohort — the standard risk-adjustment covariate layer for outcome
...
profile for Greenwood EHR encounters and aggregated cohort summaries. It reads three objects in
...
SELECT
  ...
  e.solid_tumor,
  e.mets
FROM greenwood_dbw_catalog.greenwood_ehr.elixhauser_encounter AS e
INNER JOIN greenwood_dbw_catalog.greenwood_ehr.pop_readmit_30d_tbl AS c
...
FROM greenwood_dbw_catalog.greenwood_ehr.elixhauser_encounter
```
...
SELECT
  ...
  avg_los,
  avg_cost
FROM greenwood_dbw_catalog.greenwood_ehr.population_elixhauser_summary
...
SELECT
  comorbidity_key,
  icd10_prefix
FROM greenwood_dbw_catalog.greenwood_ehr.elixhauser_icd10_map
...
- `greenwood_dbw_catalog.greenwood_ehr.elixhauser_encounter` — encounter-level AHRQ Elixhauser
...
- `greenwood_dbw_catalog.greenwood_ehr.population_elixhauser_summary` — pre-aggregated comorbidity
...
- `greenwood_dbw_catalog.greenwood_ehr.elixhauser_icd10_map` — reference mapping from AHRQ
