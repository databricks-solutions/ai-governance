...
description: Resolve clinical free-text concepts ('blood thinners', 'CT PE', 'sepsis bundle') to
  coded values and condition definitions from greenwood_ehr code_value and condition_spec — the
...
coded values and condition definitions that drive cohort queries across the Greenwood EHR semantic
layer. It reads two objects in `greenwood_dbw_catalog.greenwood_ehr`:

- **`code_value`** — the EHR vocabulary table mapping `code_value` identifiers to
...
SELECT
  ...
  display,
  concept_domain
FROM greenwood_dbw_catalog.greenwood_ehr.code_value
...
- **Condition not in condition_spec** — `condition_spec` covers the 20 flagship conditions in
  the Greenwood EHR semantic layer. If the requested condition is absent, inform the user and suggest
...
- `greenwood_dbw_catalog.greenwood_ehr.code_value` — EHR vocabulary: coded identifiers,
  display labels, code set groupings, and concept domains. No patient data.
- `greenwood_dbw_catalog.greenwood_ehr.condition_spec` — clinical-condition registry: ICD-10
