...
description: Analyze sepsis cohort outcomes (in-hospital mortality, ICU transfer, LOS) and
  SEP-1 bundle compliance from the Greenwood EHR semantic layer — reusing the governed pop_sepsis
...
A Tier-3 clinical outcome skill: it analyzes sepsis encounter outcomes across the Greenwood EHR
...
FROM greenwood_dbw_catalog.greenwood_ehr.pop_sepsis_tbl
...
- `greenwood_dbw_catalog.greenwood_ehr.pop_sepsis_tbl` — governed sepsis cohort table, materialized
...
- `greenwood_dbw_catalog.greenwood_ehr.condition_spec` — clinical-condition registry; the `sepsis`
