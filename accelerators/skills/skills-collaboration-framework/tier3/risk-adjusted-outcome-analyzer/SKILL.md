...
description: Analyze cross-cohort risk-adjusted mortality (RAMR) and observed-vs-expected (O/E)
  ratios using the VCH Cerner GLMM machinery — facility random effects, van Walraven adjustment,
...
observed-vs-expected (O/E) ratios across any registered inpatient cohort using the VCH Cerner
...
- **`predict_glmm_cohort(population_csv STRING, lookback_years INT)`** — accepts a
  **single** registered condition key and a lookback window (in years); to analyze multiple
  ...
  Call as `SELECT ... FROM greenwood_dbw_catalog.vch_cerner.predict_glmm_cohort('sepsis', 2)`.
...
FROM greenwood_dbw_catalog.vch_cerner.mv_facility_ramr
...
FROM greenwood_dbw_catalog.vch_cerner.glmm_predictions
GROUP BY facility
ORDER BY oe_ratio DESC
```
...
FROM (
  SELECT * FROM greenwood_dbw_catalog.vch_cerner.predict_glmm_cohort('sepsis', 2)
  UNION ALL
  SELECT * FROM greenwood_dbw_catalog.vch_cerner.predict_glmm_cohort('stroke_mi', 2)
)
...
SELECT
  ...
  ROUND(SUM(observed) / NULLIF(SUM(predicted_prob), 0), 3)          AS oe_ratio,
  ROUND(AVG(vw_score), 2)                                           AS avg_vw_score
FROM greenwood_dbw_catalog.vch_cerner.predict_glmm_cohort('sepsis', 2)
...
FROM greenwood_dbw_catalog.vch_cerner.facility_ramr_lookback(2)
ORDER BY oe_ratio DESC
```
...
- `greenwood_dbw_catalog.vch_cerner.glmm_predictions` — per-encounter GLMM prediction table
...
- `greenwood_dbw_catalog.vch_cerner.mv_facility_ramr` — governed RAMR metric view (columns
...
- `greenwood_dbw_catalog.vch_cerner.predict_glmm_cohort(population_csv, lookback_years)` —
...
- `greenwood_dbw_catalog.vch_cerner.facility_ramr_lookback(lookback_years)` — TVF
  (live-verified) returning facility-level RAMR summary: `facility`, `n_encounters`,
  `observed_deaths`, `expected_deaths`, `observed_rate`, `expected_rate`, `oe_ratio`,
  `risk_adjusted_mortality_rate`, `overall_crude_rate`, `avg_vw_score`.
- `greenwood_dbw_catalog.vch_cerner.population_endpoint_features` — optional supplementary
