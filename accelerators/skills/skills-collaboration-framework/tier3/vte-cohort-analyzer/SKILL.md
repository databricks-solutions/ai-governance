...
description: Analyze hospital-acquired VTE (DVT/PE) outcomes, prophylaxis adherence, and PSI-12
  quality metrics from the VCH Cerner semantic layer — and build the governed pop_vte cohort
...
events across inpatient encounters using the VCH Cerner semantic layer, and documents how to
...
SELECT
  ...
  COUNT(*)                  AS n_orders
FROM greenwood_dbw_catalog.vch_cerner.orders
...
FROM greenwood_dbw_catalog.vch_cerner.population_endpoint_features AS f
...
`pop_vte` does not currently exist as a registered population in the VCH Cerner semantic layer.
...
```sql
CREATE OR REPLACE FUNCTION greenwood_dbw_catalog.vch_cerner.pop_vte(
  cond_key STRING,
  cmp_pe   INT
)
RETURNS BOOLEAN
...
```sql
CREATE OR REPLACE TABLE greenwood_dbw_catalog.vch_cerner.pop_vte_tbl AS
SELECT
  encntr_id,
  person_id,
  cond_key,
  ...
  race_cd
FROM greenwood_dbw_catalog.vch_cerner.population_endpoint_features
WHERE greenwood_dbw_catalog.vch_cerner.pop_vte(cond_key, cmp_pe) = true
  AND is_inpatient = 1
```
...
```sql
CREATE OR REPLACE VIEW greenwood_dbw_catalog.vch_cerner.mv_vte_endpoint AS
SELECT
  loc_facility_cd,
  DATE_TRUNC('month', admit_month)                              AS month,
...
FROM greenwood_dbw_catalog.vch_cerner.pop_vte_tbl
GROUP BY loc_facility_cd, DATE_TRUNC('month', admit_month)
```
...
```sql
INSERT INTO greenwood_dbw_catalog.vch_cerner.population_definition
...
- `greenwood_dbw_catalog.vch_cerner.condition_spec` — clinical-condition registry; the `pe`
...
- `greenwood_dbw_catalog.vch_cerner.orders` — order-level records with `catalog_cd` for
...
- `greenwood_dbw_catalog.vch_cerner.pop_vte_tbl` — governed VTE cohort table (created by the
