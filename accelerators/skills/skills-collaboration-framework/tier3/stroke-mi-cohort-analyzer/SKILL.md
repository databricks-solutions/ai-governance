...
description: Analyze ischemic stroke and acute MI cohort outcomes (30-day mortality, 30-day
  readmission, reperfusion therapy rates) from the VCH Cerner semantic layer — reusing the
...
(AMI) encounter outcomes across the VCH Cerner inpatient population using the governed semantic
layer. It reads three objects in `greenwood_dbw_catalog.vch_cerner`:
...
FROM greenwood_dbw_catalog.vch_cerner.pop_stroke_mi_tbl
...
- `greenwood_dbw_catalog.vch_cerner.pop_stroke_mi_tbl` — governed stroke/MI cohort table,
...
- `greenwood_dbw_catalog.vch_cerner.condition_spec` — clinical-condition registry; the `stroke`
